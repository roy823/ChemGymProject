...
SAVE_DIR = CHECKPOINTS / "week6_mask_sweep_report"
...
def collect_train_rows(group: SweepGroup) -> List[Dict]:
    dedup: Dict[int, Dict] = {}
...
def write_markdown(path: Path, detail_rows: List[Dict], summary_rows: List[Dict]) -> None:
    lines: List[str] = []
    lines.append("# Week 6 Mask Sweep Report")
    lines.append("")
    lines.append("This report tracks the feasible-performance tradeoff along the composition-envelope axis at mu_CO = -0.6 eV.")
    lines.append("")

    if summary_rows:
        lines.append("## Summary")
        lines.append("")
        lines.append("| label | budget | max_deviation | n_seeds | mean(best_omega) | mean(feasible_best_omega) | best_feasible_frac | mean(gap) | valid_frac | mean(d_frac) |")
        lines.append("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |")
        for row in sorted(summary_rows, key=lambda r: (r["budget_steps"], 999 if row_value_none(r["max_deviation"]) else r["max_deviation"], r["label"])):
    ...
    if detail_rows:
        ...
        lines.append("")
        lines.append("| label | train_seed | budget | max_deviation | mean(best_omega) | mean(feasible_best_omega) | gap | valid_frac | source_dir |")
        lines.append("| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- |")
        for row in sorted(detail_rows, key=lambda r: (r["budget_steps"], 999 if row_value_none(r["max_deviation"]) else r["max_deviation"], r["train_seed"])):
            md = "open/fixed" if row_value_none(row["max_deviation"]) else str(int(row["max_deviation"]))
            lines.append(
                "| "
                f"{row['label']} | "
...
def main() -> None:
    ...
    write_csv(SAVE_DIR / "sweep_summary.csv", summary_rows)
    write_markdown(SAVE_DIR / "REPORT.md", detail_rows=detail_rows, summary_rows=summary_rows)

    print(SAVE_DIR / "per_train_seed_metrics.csv")
    print(SAVE_DIR / "sweep_summary.csv")
